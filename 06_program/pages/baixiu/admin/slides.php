<?php 
  ...
  if(!empty($_POST)){ //添加
    ...
    $result = update('options',array('value'=>$json),10);// $result = update('options', array('home_slides'=>$json), 10);

    if($result){
      header('Location: /admin/slides.php');exit;
    }
  }
...
<!-- 这个添加操作：不是说添加了一条数据，而是在一个数据中添加了部分内容，是在一个字符串中追加了一些内容。
在前面把所有的数据取出，把它变成一个php的数组，然后往数组中去追加一个单元
真正的数据时放在value字段下的 -->
