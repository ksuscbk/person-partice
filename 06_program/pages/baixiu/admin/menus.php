<?php 
  ...
  if($action == 'delete'){ // 删除操作
    ...
    if($result){
      header('Location: /admin/menus.php');exit;
    }
  }
  ...
  if(!empty($_POST)){ // 添加操作
    ...
    // 执行更新操作
    update('options',array('value'=>$json), 9);
...
<!-- 整个列表数据，是放到一个字段下的，只不过是一个json格式的数据，表面上看是删除操作，其实是修改操作，在修改的时候，需要对json转来转去（从数据库中取出是json字符串，操作的时候需要转成数组，操作过程中，要删除一条数据，其实是在数组中删除一个单元，删除完之后还要把数据存回去，就需要再转回字符串，遇到bug，通过重复赋值解决了）  -->
