<?php 
  ...
  // 获取action值，根据action的值，决定处理哪些（增删改查）逻辑
  ...
  if($action == 'add'){ //添加
    ...
    // 只有点击添加按钮时才有必要操作数据库
    if(!empty($_GET)){ //不为空的时候才有必要去存数据库，否则只定义上面两个变量就可以
      unset($_GET['id']);
      unset($_GET['action']);
...
<body>
  ...
  <div class="main">
    ...
    <div class="container-fluid">
      ...
      <!-- 有错误信息时展示 -->
      <!-- 只有走到update的时候才会有$message变量,一开始是add的时候没有这个变量,就会报错。 -->
      <!-- 也就是说$message不为空的时候,就不会报错 -->
      ...
      <div class="row">
        <div class="col-md-4">
          ...
          <form action="/admin/categories.php" method="get">
            ...
            <!-- 作用：这个input能够提交数据，但是不能再页面展示 -->
            <!-- 一个隐藏的input，相当于下面这种形式,zi加/为了好注释 -->
            <!-- <input type="text" style="display: none;"> name="id" value="< / ?php echo $cat_id; ?>"> -->
